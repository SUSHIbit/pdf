...
<head>
    ...
    <title>Admin Panel - {{ config('app.name', 'AI Document Q&A') }}</title>
        ...
        <header class="fixed inset-x-0 top-0 z-50 bg-surface/95 dark:bg-surface-dark/95 backdrop-blur-sm border-b border-border dark:border-border-dark shadow-sm transition-colors duration-300">
            <nav class="flex items-center justify-between p-4 lg:px-8 max-w-7xl mx-auto">
                ...
                <div class="flex items-center">
                    <a href="{{ route('admin.dashboard') }}" class="text-xl md:text-2xl font-bold text-text-primary dark:text-text-dark-primary transition-colors">
                ...
                <div class="hidden md:flex items-center space-x-4">
                    <!-- Navigation Links -->
                    <a href="{{ route('admin.dashboard') }}" class="text-base font-medium text-text-primary dark:text-text-dark-primary hover:text-text-secondary dark:hover:text-text-dark-secondary transition-colors px-3 py-2 {{ request()->routeIs('admin.dashboard') ? 'bg-primary-100 dark:bg-primary-800 rounded-lg' : '' }}">
                        Dashboard
                    </a>
                    <a href="{{ route('admin.users.index') }}" class="text-base font-medium text-text-primary dark:text-text-dark-primary hover:text-text-secondary dark:hover:text-text-dark-secondary transition-colors px-3 py-2 {{ request()->routeIs('admin.users.*') ? 'bg-primary-100 dark:bg-primary-800 rounded-lg' : '' }}">
                        User Management
                    </a>
                    ...
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open" class="flex items-center space-x-2 text-text-primary dark:text-text-dark-primary hover:text-text-secondary dark:hover:text-text-dark-secondary transition-colors p-2 rounded-lg hover:bg-primary-100 dark:hover:bg-primary-800">
                            <div class="w-8 h-8 bg-primary-600 dark:bg-primary-500 rounded-full flex items-center justify-center">
                                <span class="text-sm font-medium text-white">{{ substr(auth('admin')->user()->name, 0, 1) }}</span>
                            </div>
                            <span class="hidden lg:block">{{ auth('admin')->user()->name }}</span>
                        ...
                        <div x-show="open" @click.away="open = false" 
                            ...
                            <a href="{{ route('home') }}" class="block px-4 py-2 text-sm text-text-primary dark:text-text-dark-primary hover:bg-primary-50 dark:hover:bg-primary-900/20 transition-colors">Main Site</a>
            ...
            <div x-show="mobileMenuOpen" 
                ...
                <div class="px-4 py-3 space-y-1 max-w-7xl mx-auto">
                    <a href="{{ route('admin.dashboard') }}" 
                       ...
                       class="block px-4 py-3 text-base font-medium text-text-primary dark:text-text-dark-primary hover:bg-primary-50 dark:hover:bg-primary-900/20 rounded-lg transition-colors {{ request()->routeIs('admin.dashboard') ? 'bg-primary-100 dark:bg-primary-800' : '' }}">
                    ...
                    <a href="{{ route('admin.users.index') }}" 
                       @click="closeMobileMenu()"
                       class="block px-4 py-3 text-base font-medium text-text-primary dark:text-text-dark-primary hover:bg-primary-50 dark:hover:bg-primary-900/20 rounded-lg transition-colors {{ request()->routeIs('admin.users.*') ? 'bg-primary-100 dark:bg-primary-800' : '' }}">
                        <svg class="w-5 h-5 inline mr-3" fill="currentColor" viewBox="0 0 20 20">
